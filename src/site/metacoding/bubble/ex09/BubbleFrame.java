...
/**
 * 목적: 버블발사하기 (충돌 감지 안함) 수평발사!! 버블이 수평 이동!!
 */
...
public class BubbleFrame extends JFrame {
	...
	private void initObject() {
		...
		player = new Player(context); // player가 jlabel이니까 해도 되
		add(player);
		...
		addKeyListener(new KeyListener() { // 타입은 keylistener넣을 수 있다.
			...
			public void keyPressed(KeyEvent e) { // 키보드를 누르면
				...
				if (e.getKeyCode() == 39) { // 키보드를 누르고 있는 동안 right메서드를 한번만 실행하고 싶다!!
					...
					if (player.isUp() == false && player.isDown() == false) { // 이벤트 루프에 등록된 후 2초 지나서 검사하니까 무조건 false다.
						player.up();
					}
